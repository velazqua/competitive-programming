...
bigNum multiply(bigNum A, bigNum B, vector<string> &M)
{
	bigNum C("0");
	int currMult;
	int carry;
	...
	for( int i=0; i<T; ++i )
	{
		...
		switch( optr )
		{
			case '+': 
			{
				...
				cout << printSpaces( num1.size(), numSpaces ) << num1 << endl;
				cout << printSpaces( num2.size()+1, numSpaces ) << optr << num2 << endl;
				cout << printSpaces( numDashes, numSpaces ) << printDashes( numDashes ) << endl;
				cout << printSpaces( result.s, numSpaces ) << result << endl;
				break;
			}
			case '-': 
			{
			...
			{
				vector<string> Ms;
				bigNum result = multiply( bigNum( num1 ), bigNum( num2 ), Ms );
				int numSpaces = max( max( num1.size(), num2.size()+1 ), result.number.size() );
				...
				if( num2.size() > 1 )
				{
					...
					cout << printSpaces( numDashes1, numSpaces ) << printDashes( numDashes1 ) << endl;
					int size = Ms.size();
					for( int k=0; k<size; ++k )
					{
						cout << printSpaces( Ms[k].size()+k, numSpaces ) << Ms[k] << endl;
					}
					cout << printSpaces( numDashes2, numSpaces ) << printDashes( numDashes2 ) << endl;
					result.updateS();
					cout << printSpaces( removeLeadingZeroes(result.number).size(), numSpaces ) << removeLeadingZeroes(result.number) << endl;
					
					}
				else
				{
					cout << printDashes( numDashes2 ) << endl;
					result.updateS();
					cout << printSpaces( removeLeadingZeroes(result.number).size(), numDashes2 ) << removeLeadingZeroes(result.number) << endl;
				}
				break;
			}
		}
