/* Name: Alex Velazquez
 * Start Date: August 27, 8:51PM
 * End Date:   ?????????????????
 * Purpose: To solve the Project Euler problem #104
 * Link:http://projecteuler.net/index.php?section=problems&id=104
 * */
...
bool lastPandigital (string const &S)
{
	set<char> digits;
	int L = (int)S.size();
	pair<set<char>::iterator,bool> result;
	for (int i=L-1;i>L-1-9;--i)
	{
		if (S[i] == '0') return false;
		result = digits.insert(S[i]);
		if (result.second == false)	return false;
	}
	return true;
}

bool firstPandigital (string const &S)
{
	set<char> digits;
	pair<set<char>::iterator,bool> result;
	for (int i=0;i<9;++i)
...
int main ()
{
	...
	for (int i=3;i<500;++i)
	{
		fib[i] = bigNum(fib[i-1].number) + bigNum(fib[i-2].number);
	}
	for (int i=500;i<LIMIT;++i)
	{
		fib[i] = bigNum(fib[i-1].number) + bigNum(fib[i-2].number);
		if (lastPandigital(fib[i].number) && firstPandigital(fib[i].number))
		{
			cout << i << endl;
			break;
		}
